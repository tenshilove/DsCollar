...
UserCommand(integer iNum, string sStr, key kID, integer reMenu) {
    ...
    if (~llListFindList(commands, [llList2String(lParams,0)]) || (llList2String(lParams,0)=="menu" && ~llListFindList(commands, [llList2String(lParams,1)])) ) {
        if (kID == g_kWearer || iNum == CMD_OWNER) {
            ...
            if (sCommand == "themes" || sStrLower == "menu themes") {
                ...
                if (sElement == g_sDeviceType) g_iCollarHidden = !iCurrentlyShown;
                    ...
                    if (sLinkType == sElement || sElement==g_sDeviceType) {
                        if (!g_iCollarHidden || sElement == g_sDeviceType ) {
                            ...
                            if (iCurrentlyShown){
                                ...
                                } else if (~iGlowsIndex) g_lGlows = llDeleteSubList(g_lGlows,iGlowsIndex,iGlowsIndex+1);
                                llSetLinkPrimitiveParamsFast(iLinkCount, [PRIM_GLOW, ALL_SIDES, 0.0]);
                            }
                        }
                    }
                }
