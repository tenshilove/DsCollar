...
list g_lDescriptionsOn = [
    "May send or start IMs even when blocked",
    "May receive IMs even when blocked",
    "May see local chat even when blocked",
    "May see emotes even when blocked",
    "May receive teleport offers even when blocked",
    "Wearer may not refuse a teleport offer"
];
list g_lDescriptionsOff =[
    "Sending and starting IMs to them can be blocked",
    "Receiving IMs may be blocked",
    "Local chat may be blocked",
    "Emotes may be blocked",
    "Teleport offers may be declined",
    "Wearer may refuse TP offers"
        ];
...
UserCommand(integer iNum, string sStr, key kID) {
    ...
    for (; iL < iInd; iL += 2) {
        ...
        if (~llListFindList(g_lOwners, [sWho])) {
            llMessageLinked(LINK_DIALOG,NOTIFY,"0"+"You may not set different exceptions from other Owners for "+sWhoName + ", unless you use terminal.",kID);
            jump nextwho;
        } else if (~llListFindList(g_lSecOwners, [sWho])) {
            llMessageLinked(LINK_DIALOG,NOTIFY,"0"+"You may not set different exceptions from other Trustees for "+sWhoName + ", unless you use terminal.",kID);
            jump nextwho;
        }
...
Menu(key kID, string sWho, integer iAuth) {
    if (!g_iRLVOn) {
        llMessageLinked(LINK_DIALOG,NOTIFY,"0"+"RLV features are now disabled in this %DEVICETYPE%. You may enable them in the RLV submenu. Opening it now.",kID);
        llMessageLinked(LINK_RLV, iAuth, "menu RLV", kID);
        return;
    }
    list lButtons = ["Owner", "Trustee"];
    string sPrompt = "\n[Exceptions]\n\nSet exceptions to RLV restrictions.\n\n(\"Force Teleports\" are already on by default for Owners.)";
