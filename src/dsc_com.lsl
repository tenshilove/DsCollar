...
integer g_iNeedsPose = FALSE;
string g_sPOSE_ANIM = "turn_180";

integer g_iTouchNotify = FALSE;
integer g_iHighlander = TRUE;
list g_lCore5Scripts = ["LINK_AUTH","dsc_auth","LINK_DIALOG","dsc_dialog","LINK_RLV","dsc_rlvsys","LINK_SAVE","dsc_settings","LINK_ANIM","dsc_anim","LINK_ANIM","dsc_couples"];
...
default {
    ...
    listen(integer iChan, string sName, key kID, string sMsg) {
        if (iChan == g_iHUDChan) {
            ...
            } else if (iChan == g_iInterfaceChannel && llGetOwnerKey(kID) == g_kWearer) {
        ...
        if(llGetOwnerKey(kID) == g_kWearer) {
            ...
            if (sw == g_sSafeWord) {
                ...
                llMessageLinked(LINK_DIALOG,NOTIFY,"0"+"You have safeworded. Your Owners have been notified.",g_kWearer);
                llMessageLinked(LINK_DIALOG,NOTIFY_OWNERS,"\n\n%WEARERNAME% had to safeword. Please check on %WEARERNAME%'s well-being in case further care is required.\n","");
                return;
            }
        }
        ...
        if (iChan == g_iInterfaceChannel) {
            ...
            if (sMsg == "OpenCollar?") llRegionSayTo(g_kWearer, g_iInterfaceChannel, "OpenCollar=Yes");
                ...
                llOwnerSay("\n\nWARNING: You are attempting to wear more than one script core. This may cause errors with other compatible accessories, and interfere with your RLV relay. For a smooth experience, and to avoid wearing unnecessary script duplicates, please consider taking off \""+sName+"\" manually if it doesn't detach automatically.\n");
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID) {
        if (iNum >= CMD_OWNER && iNum <= CMD_WEARER) UserCommand(kID, iNum, sStr);
            ...
            if (llGetInventoryType("dsc_relay") == INVENTORY_SCRIPT) {
                if (!llGetScriptState("dsc_relay")) {
                    llSetScriptState("dsc_relay",TRUE);
                    llResetOtherScript("dsc_relay");
                }
            }
            llResetScript();
        }
