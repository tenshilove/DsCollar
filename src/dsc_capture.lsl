...
doCapture(string sCaptorID, integer iIsConfirmed) {
    ...
    if (!iIsConfirmed) {
        ...
        llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"You are at "+NameURI(sCaptorID)+"'s mercy.",g_kWearer);
        llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"\n\n%WEARERNAME% is at your mercy now.\n\nNOTE: During capture RP %WEARERNAME% is unable to refuse your teleport offers, and you will be in full control. Type \"/%CHANNEL% %PREFIX% leash\" to clip a leash, \"/%CHANNEL% %PREFIX% unleash\" to unleash it, or \"/%CHANNEL% %PREFIX% capture release\" to relinquish capture access to %WEARERNAME%'s %DEVICETYPE%.\n\nHave fun!.\n", sCaptorID);
    ...
    if (llSubStringIndex(sStr,"capture TempOwner") == 0){
        ...
        if (g_sTempOwnerID != "" && kID==g_kWearer) {
            ...
            if (g_iRiskyOn && g_iCaptureInfo) {
                llMessageLinked(LINK_DIALOG,SAY,"1"+"%WEARERNAME%: If you touch my %DEVICETYPE% you can capture me...","");
            ...
            llMessageLinked(LINK_DIALOG,NOTIFY,"0"+NameURI(kID)+" has freed you.",g_kWearer);
            ...
            if (~iMenuIndex) {
                ...
                if (sMenu=="CaptureMenu") {
                    ...
                    if (sMessage == "BACK") UserCommand(iNum, "menu capture", kID, FALSE);
                        ...
                        llMessageLinked(LINK_DIALOG,NOTIFY,"0"+NameURI(kCaptor)+" didn't pass your face control. Too bad for them.",kAv);
                        llMessageLinked(LINK_DIALOG,NOTIFY,"0"+"Apparently, %WEARERNAME% didn't want to be captured after all.",kCaptor);
                    }
