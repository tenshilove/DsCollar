...
RootActionsMenu(key kAv, integer iAuth) {
    ...
    string sPrompt = "\n[DsCollar - RLV Folders]\n\nYou are at the #RLV shared folders root.\n\nHere, you may restrict wearing or removing items not in the shared folder tree.\nYou can also save or load the list of currently worn shared folders.\n\nWhat do you want to do?";
...
FolderBrowseMenu(string sStr) {
    ...
    if (g_sCurrentFolder!="") {
        ...
        if (iWorn / 10 == 1 ) sPrompt += "Folder can be worn";
        else if (iWorn / 10 == 2 ) sPrompt += "Folder can be worn or unworn";
        else if (iWorn / 10 == 3 ) sPrompt += "Folder can be unworn";
        else if (iWorn / 10 == 0 ) sPrompt += "Folder does not contain wearables";
        sPrompt += ".\n";
    ...
    sPrompt += "\n- Click "+ACTIONS_CURRENT+" to manage this folder's content.\n- Click one of the subfolders to browse it.\n";
    if (g_sCurrentFolder!="") {sPrompt += "- Click "+PARENT+" to ho back to this folder's parent.\n"; lUtilityButtons += [PARENT];}
                ...
                if (sMenu == "History") {
                    ...
                    if (sMessage == UPMENU) {
                        ...
                        if (g_iUnsharedLocks & 0x1) {
                            ...
                            llMessageLinked(LINK_DIALOG,NOTIFY,"1"+"Removing unshared items is no longer forbidden now.",kAv);
                        } else {
                            updateUnsharedLocks(0x1, 0x0);
                            llMessageLinked(LINK_DIALOG,NOTIFY,"1"+"Removing unshared items is forbidden now.",kAv);
                        }
                    } else if (sMessage == lockUnsharedButton(1, 0)) {
                        if (g_iUnsharedLocks & 0x2) {
                            updateUnsharedLocks(0x0, 0x2);
                            llMessageLinked(LINK_DIALOG,NOTIFY,"1"+"Wearing unshared items is no longer forbidden now.",kAv);
                        } else {
                            updateUnsharedLocks(0x2, 0x0);
                            llMessageLinked(LINK_DIALOG,NOTIFY,"1"+"Wearing unshared items is forbidden now.",kAv);
                        }
                    }
                    ...
                    if (sMessage != UPMENU) { addToHistory(g_sCurrentFolder); llSleep(1.0);} 
