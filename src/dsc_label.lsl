...
Changed for the OpenCollar label, only one face per prim on a cut cylinder,
...
Debug(string sStr) {
    ...
    //if you delete the block comment marks above and below
...
// find all 'Label' prims, count and store its link numbers for fast work SetLabel() and timer
...
integer LabelsCount() {
    ...
    //find all 'Label' prims and store its links to list
...
SetLabel() {
    ...
    if(g_iScroll==TRUE) 
	{// || llStringLength(g_sLabelText) > g_iCharLimit)
        // add some blanks
