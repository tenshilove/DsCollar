    ...
    if (sCommand == "menu") {
        ...
        if ( sValue=="on") {
            if (iNum<=CMD_WEARER) {
                ...
                llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"Your neck is restrained by a posture %DEVICETYPE%.",g_kWearer);
                if (kID != g_kWearer) llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"%WEARERNAME%'s neck is restrained by a posture %DEVICETYPE%.", kID);
            ...
            if (iNum<=g_iLastPostureRank) {
                ...
                llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"You may freely look around again.",g_kWearer);
                if (kID != g_kWearer) llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"%WEARERNAME% is free to look around again.", kID);
            }
                else llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"%NOACCESS%",kID);
        }
        ...
        if (sValue=="on") {
            if (iNum<=CMD_WEARER) {
                ...
                llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"Only an Owner may change or stop your positions now.",g_kWearer);
                if (kID != g_kWearer) llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"%WEARERNAME%'s position may only be changed or released by an Owner.", kID);
            } else llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"%NOACCESS%", kID);
        } else if (sValue=="off") {
            if (iNum<=g_iLastPoselockRank) {
                ...
                llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"You may now freely change or stop positions.",g_kWearer);
                if (kID != g_kWearer) llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"%WEARERNAME% may freely change or release positions.", kID);
            } else llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"%NOACCESS%", kID);
        }
    } else if (sCommand == "ao") {
        ...
        if ((iNum == CMD_OWNER)||(kID == g_kWearer)) {
            ...
            if (sValue == "on") {
                ...
                } else llMessageLinked(LINK_DIALOG, NOTIFY, "1"+"\n\nAntislide may not be used when a server-side AO is already running. If you are wearing the OpenCollar AO, it will take care of this functionality on its own and Antislide is not required.\n", kID);
            } else if (sValue == "off") {
                g_iTweakPoseAO = 0;
            ...
            else llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"Can not find animation "+llList2String(g_lPoseMoveAnimationPrefix,0)+sValueNotLower, kID);
        } else llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"Only Owners or the %DEVICETYPE% wearer may change antislide settings.",g_kWearer);
...
StopAnim(string sAnim) {
    ...
    } else  llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"Error: Somehow your %DEVICETYPE% lost permission to animate you. Try taking it off and re-attaching it.",g_kWearer);
}
...
integer SetPosture(integer iOn, key kCommander) {
    ...
    if (llGetPermissions() & PERMISSION_TRIGGER_ANIMATION) {
        ...
        llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"Error: Somehow your %DEVICETYPE% lost permission to animate you. Try taking it off and re-attaching it.",g_kWearer);
        return FALSE;
    }
}
...
AnimMenu(key kID, integer iAuth) {
    ...
    if (g_iAnimLock) {
        sPrompt += " may not freely change or release postions";
        lButtons = ["☑ AnimLock"];
    } else {
        sPrompt += " may freely change or release positions";
        lButtons = ["☐ AnimLock"];
    }
    if (llGetInventoryType("~stiff")==INVENTORY_ANIMATION) {
        if (g_iPosture) {
            sPrompt +=" and is wearing a posture %DEVICETYPE%.";
            lButtons += ["☑ Posture"];
        } else {
            sPrompt +=" and is not wearing a posture %DEVICETYPE%.";
            lButtons += ["☐ Posture"];
        }
    }
            ...
            if (~iMenuIndex) {
                ...
                if (sMenuType == "Anim") {
                    ...
                    if (sMessage == "BACK")
                        ...
                        llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"\n\nAttempting to trigger the AO menu. This will only work if %WEARERNAME% is using an OpenCollar AO or an AO Link script in their AO HUD.", kAv);
                        AOMenu(kAv, iAuth);
                    } else {
